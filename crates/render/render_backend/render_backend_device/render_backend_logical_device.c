...
DResult render_backend_create_logical_device(RenderBackend *backend)
{
  ...
  vkGetDeviceQueue(backend->device.logical_device, indices.transfer_family, 0, &backend->device.transfer_queue);

  backend->device.graphics_family = indices.graphics_family;
  backend->device.present_family = indices.present_family;
  backend->device.transfer_family = indices.transfer_family;
